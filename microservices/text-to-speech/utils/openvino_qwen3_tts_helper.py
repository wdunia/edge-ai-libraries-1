...
if TORCH_AVAILABLE and DYNAMIC_LAYER_AVAILABLE:
...
if TORCH_AVAILABLE:
    if hasattr(ALL_MASK_ATTENTION_FUNCTIONS, "register"):
        ...
        ALL_MASK_ATTENTION_FUNCTIONS["sdpa"] = eager_mask_without_vmap
...
def qwen_openvino_model_exists(output_dir: str | Path) -> bool:
    model_dir = Path(output_dir)
    required_files = [
        "config.json",
        "checkpoint_path.txt",
        TALKER_LANGUAGE_NAME,
        TALKER_LANGUAGE_NAME.replace(".xml", ".bin"),
        TALKER_EMBEDDING_NAME,
        TALKER_EMBEDDING_NAME.replace(".xml", ".bin"),
        TALKER_TEXT_EMBEDDING_NAME,
        TALKER_TEXT_EMBEDDING_NAME.replace(".xml", ".bin"),
        TALKER_TEXT_PROJECTION_NAME,
        TALKER_TEXT_PROJECTION_NAME.replace(".xml", ".bin"),
        TALKER_CODE_PREDICTOR_EMBEDDING_NAME,
        TALKER_CODE_PREDICTOR_EMBEDDING_NAME.replace(".xml", ".bin"),
        TALKER_CODE_PREDICTOR_NAME,
        TALKER_CODE_PREDICTOR_NAME.replace(".xml", ".bin"),
        Path("speech_tokenizer") / "config.json",
        Path("speech_tokenizer") / SPEECH_TOKENIZER_ENCODER_NAME,
        Path("speech_tokenizer") / SPEECH_TOKENIZER_ENCODER_NAME.replace(".xml", ".bin"),
        Path("speech_tokenizer") / SPEECH_TOKENIZER_DECODER_NAME,
        Path("speech_tokenizer") / SPEECH_TOKENIZER_DECODER_NAME.replace(".xml", ".bin"),
    ]
    return all((model_dir / relative_path).exists() for relative_path in required_files)


def export_qwen3_tts_openvino_model(
    model_id: str,
    output_dir: str | Path,
    ...
    if qwen_openvino_model_exists(model_dir):
        return
    ...
    checkpoint_path_file.write_text(f"{model_id}\n", encoding="utf-8")
...
def convert_qwen3_tts_model(model_id, output_dir, quantization_config=None, use_local_dir=False):
    ...
    speech_tokenizer_files_ready = all(
        [
            (speech_tokenizer_dir / "config.json").exists(),
            (speech_tokenizer_dir / SPEECH_TOKENIZER_ENCODER_NAME).exists(),
            (speech_tokenizer_dir / SPEECH_TOKENIZER_DECODER_NAME).exists(),
        ]
    )

    if talker_files_ready and speech_tokenizer_files_ready:
        print(f"✅ {model_id} model already converted. You can find results in {output_dir}")
        return

    if talker_files_ready and not speech_tokenizer_files_ready:
        print(f"ℹ️ Found partial cached conversion for {model_id}. Reusing talker IRs and resuming speech tokenizer export.")
...
def convert_speech_tokenizer(model_id, output_dir, use_local_dir=False):
    ...
    if not encoder_path.exists():
        ...
        # Keep tokenizer audio models in full precision; FP16-compressed vocoder weights
        # noticeably degrade audibility for Qwen TTS output.
        ov.save_model(ov_model, encoder_path, compress_to_fp16=False)
        del ov_model
        cleanup_torchscript_cache()
        gc.collect()
    ...
    if not decoder_path.exists():
        ...
        try:
            ...
            # Keep tokenizer audio models in full precision; FP16-compressed vocoder weights
            # noticeably degrade audibility for Qwen TTS output.
            ov.save_model(ov_model, decoder_path, compress_to_fp16=False)
            del ov_model, traced
            cleanup_torchscript_cache()
            gc.collect()
        ...
        **kwargs,
        ...
        if encoder_path.exists() and decoder_path.exists():
            ...
            try:
                ...
                tokenizer_config = Qwen3TTSConfig.from_pretrained(model_dir)
                tokenizer_checkpoint = {
                    "qwen3_tts_tokenizer_12hz": "Qwen/Qwen3-TTS-Tokenizer-12Hz",
                    "qwen3_tts_tokenizer_25hz": "Qwen/Qwen3-TTS-Tokenizer-25Hz",
                }.get(tokenizer_config.tokenizer_type)

                if speech_tokenizer_ov_path.exists() and any(
                    (speech_tokenizer_ov_path / candidate).exists()
                    ...
                    # Try a dedicated tokenizer checkpoint first, then local sibling directories.
                    potential_paths = []
                    if tokenizer_checkpoint is not None:
                        potential_paths.append(tokenizer_checkpoint)
                    potential_paths.extend([model_dir.parent / "speech_tokenizer", model_dir / "ckpt" / "speech_tokenizer"])
