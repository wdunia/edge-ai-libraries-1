...
main() {
    ...
    echo "=== BUILDING DEMO IMAGES (one-time, self-contained, no publish step) ==="
    if command -v sg >/dev/null 2>&1 && [[ -S /var/run/docker.sock ]]; then
        sg docker -c "${SCRIPT_DIR}/build_demo_images.sh" || {
            echo "WARNING: automatic image build failed. You can retry later with:" >&2
            echo "  ./scripts/build_demo_images.sh" >&2
        }
    else
