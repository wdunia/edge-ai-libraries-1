...
SOURCE_MODE="${DLSPS_SOURCE_MODE:-rtsp}"
...
usage() {
    cat <<EOF
Usage: ./run_dlStreamer.sh [--force-restart|-f] [--open-browser] [--help]
...
Options:
  ...
  --open-browser        Run tools/autorun.py after services are up.
...
main() {
    ...
    if [[ "$SOURCE_MODE" == "file" ]]; then
        update_env_var "${UI_ENV_FILE}" "VITE_DEFAULT_STREAM_URL" "file:///home/pipeline-server/resources/videos/warehouse.avi"
    else
