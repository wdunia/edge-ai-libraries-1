...
main() {
    ...
    update_env_var "${DOCKER_ENV_FILE}" "VITE_DEFAULT_MODEL_SHARING" "${DEFAULT_MODEL_SHARING}"
    ...
    update_env_var "${UI_ENV_FILE}" "VITE_DEFAULT_MODEL_SHARING" "${DEFAULT_MODEL_SHARING}"
    ...
    echo "Demo is up: http://${ip}:${UI_PORT}"

    if [[ "$HEADLESS" == "true" ]]; then
        echo "Headless mode enabled. Open http://${ip}:${UI_PORT} manually."
        return 0
    fi

    if [[ "$AUTO_OPEN_BROWSER" == "true" ]]; then
        if ! has_graphical_session; then
            echo "No graphical session detected. Skipping automatic browser launch. Open http://${ip}:${UI_PORT} manually."
            return 0
        fi
        ...
        python3 autorun.py || echo "Skipping automatic browser launch. Open http://${ip}:${UI_PORT} manually if needed."
    fi
}
